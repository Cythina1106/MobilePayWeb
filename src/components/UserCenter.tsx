...
import { UserQuery, PendingUserQuery, UserAuditRequest, UserRejectRequest } from '../types/api'
    ...
    <div className="users-section">
      ...
      {!loading && totalUsers > 0 && (
        <div className="pagination-container" style={{
          ...
          <button
            onClick={() => setUserQuery(prev => ({ ...prev, pageNum: Math.max(1, (prev.pageNum || 1) - 1) }))}
            disabled={loading || (userQuery.pageNum || 1) <= 1}
            style={{
              padding: '8px 16px',
              backgroundColor: (userQuery.pageNum || 1) <= 1 ? '#f5f5f5' : '#007bff',
              color: (userQuery.pageNum || 1) <= 1 ? '#999' : 'white',
              border: 'none',
              borderRadius: '4px',
              cursor: (userQuery.pageNum || 1) <= 1 ? 'not-allowed' : 'pointer'
            }}
          >
            上一页
          </button>
          ...
          <span style={{ color: '#666', fontSize: '14px' }}>
            第 {userQuery.pageNum || 1} 页，共 {Math.ceil(totalUsers / (userQuery.pageSize || 10))} 页，总计 {totalUsers} 条记录
          </span>

          <button
            onClick={() => setUserQuery(prev => ({ ...prev, pageNum: (prev.pageNum || 1) + 1 }))}
            disabled={loading || (userQuery.pageNum || 1) >= Math.ceil(totalUsers / (userQuery.pageSize || 10))}
            style={{
              padding: '8px 16px',
              backgroundColor: (userQuery.pageNum || 1) >= Math.ceil(totalUsers / (userQuery.pageSize || 10)) ? '#f5f5f5' : '#007bff',
              color: (userQuery.pageNum || 1) >= Math.ceil(totalUsers / (userQuery.pageSize || 10)) ? '#999' : 'white',
              border: 'none',
              borderRadius: '4px',
              cursor: (userQuery.pageNum || 1) >= Math.ceil(totalUsers / (userQuery.pageSize || 10)) ? 'not-allowed' : 'pointer'
            }}
          >
            下一页
          </button>
    ...
    <div className="applications-section">
      ...
      {!loading && totalApplications > 0 && (
        <div className="pagination-container" style={{
          ...
          <button
            onClick={() => setPendingQuery(prev => ({ ...prev, pageNum: Math.max(1, (prev.pageNum || 1) - 1) }))}
            disabled={loading || (pendingQuery.pageNum || 1) <= 1}
            style={{
              padding: '8px 16px',
              backgroundColor: (pendingQuery.pageNum || 1) <= 1 ? '#f5f5f5' : '#007bff',
              color: (pendingQuery.pageNum || 1) <= 1 ? '#999' : 'white',
              border: 'none',
              borderRadius: '4px',
              cursor: (pendingQuery.pageNum || 1) <= 1 ? 'not-allowed' : 'pointer'
            }}
          >
            上一页
          </button>
          ...
          <span style={{ color: '#666', fontSize: '14px' }}>
            第 {pendingQuery.pageNum || 1} 页，共 {Math.ceil(totalApplications / (pendingQuery.pageSize || 10))} 页，总计 {totalApplications} 条记录
          </span>

          <button
            onClick={() => setPendingQuery(prev => ({ ...prev, pageNum: (prev.pageNum || 1) + 1 }))}
            disabled={loading || (pendingQuery.pageNum || 1) >= Math.ceil(totalApplications / (pendingQuery.pageSize || 10))}
            style={{
              padding: '8px 16px',
              backgroundColor: (pendingQuery.pageNum || 1) >= Math.ceil(totalApplications / (pendingQuery.pageSize || 10)) ? '#f5f5f5' : '#007bff',
              color: (pendingQuery.pageNum || 1) >= Math.ceil(totalApplications / (pendingQuery.pageSize || 10)) ? '#999' : 'white',
              border: 'none',
              borderRadius: '4px',
              cursor: (pendingQuery.pageNum || 1) >= Math.ceil(totalApplications / (pendingQuery.pageSize || 10)) ? 'not-allowed' : 'pointer'
            }}
          >
            下一页
          </button>
